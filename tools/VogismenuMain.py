...
class VogismenuMain(QtCore.QObject):    # Die Vererbung von QtCore.Qobject benötigen
    def __init__(self, iface):          # wir für den Aufruf self.sender() in der Methode
        ...
        try:
            ...
            QtGui.QMessageBox.critical(None, "Fehler", os.path.dirname(__file__) + os.sep + ( "vogisini.xml kann nicht gelesen werden und wird automatisch erzeugt! Das VoGIS Menü wird möglicherweise nicht richtig funktionieren!").decode("utf-8"))
            write_vogisini('V:/Geodaten/','menue','menue')
        ...
        if Flagge:  #vogisin.xml konnte gelesen werden
            ...
            if self.vogisPfad == '' or self.vogisEncoding == '' or self.vogisKBS == '':
                write_vogisini('V:/Geodaten/','menue','menue')
                #und instanzvariablen mit den werten synchronisieren
                ...
                if (dialog.exec_()):
                    ...
                    if len(auswahl) == 1:
                        ...
                        if rect == QMessageBox.Ok:
                            write_vogisini('V:/Geodaten/',self.vogisEncoding,self.vogisKBS)
                            QtGui.QMessageBox.information(None, "Achtung",("Die gespeicherten Einstellungen " + self.vogisPfad + (" werden bei der nächsten Aktualisierung des VoGIS Menüs überschrieben").decode("utf-8")))
                    else:
        ...
        #die globalen Variablen belegen
        #werden dann im Modul Projektimport verwendet
        global vogisPfad_global
        global vogisEncoding_global
        global vogisKBS_global
        vogisPfad_global = self.vogisPfad
        vogisEncoding_global = self.vogisEncoding
        vogisKBS_global = self.vogisKBS
        QtGui.QMessageBox.about(None,"GLOBAL Main", vogisEncoding_global)
        ...
        if os.path.exists(dbpfad) == 1: #sonst erzeugt open eine leer sqllitedb
            ...
            if  self.db.open():
                ...
                if abfrage.first(): #user gefunden
                    ...
                    abfrage.exec_("update qgis_user set version = '1.2.0' where user = '" + username + "'")
                    self.db.close()
                else: #user nicht gefunden, d.h. noch nicht vorhanden
                    abfrage.exec_("insert into qgis_user ("'user'", "'starts'", "'version'") values ('" + username + "', 1 , '1.2.0')")
    ...
    def doVogisHilfe(self):
        os.startfile(self.vogisPfad +  "_Allgemein/QGIS/QGIS-VoGIS-Manual.pdf")
...
class Options (QtGui.QDialog, Ui_frmOptions):
    ...
    def on_ButtonSave_clicked(self):
        ...
        if rect == QMessageBox.Ok:
            write_vogisini(self.vogisPfad,self.vogisEncoding,self.vogisKBS)
            QtGui.QMessageBox.information(None, "Achtung",("Die gespeicherten Einstellungen " + self.vogisPfad + (" werden bei der nächsten Aktualisierung des VoGIS Menüs überschrieben").decode("utf-8")))
            self.lblPath.setText(self.vogisPfad.replace("\\","/"))
        #die globalen Variablen belegen
        #werden dann im Modul Projektimport verwendet
...
def write_vogisini(mainpath,encoding,kbs):
