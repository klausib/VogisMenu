...
class GstDialogSqlite (QtWidgets.QDialog,Ui_frmGstsuche):
...
##                    self.abfrage2.exec_("SELECT DISTINCT kgnrkgname FROM kat_gem_vlbg where not (PGEM_NAME = '" + str.strip(self.Gemeinde) + "') ORDER BY KGEM_GESNR")

##                elif self.rbName.isChecked():

##                    self.abfrage2.exec_("SELECT DISTINCT kgnrkgname FROM kat_gem_vlbg where not (PGEM_NAME = '" + str.strip(self.Gemeinde) + "') ORDER BY KGEM_NAME")

##            else:

##                if self.rbKG.isChecked():

##                    self.abfrage2.exec_("SELECT DISTINCT kgnrkgname FROM kat_gem_vlbg WHERE (PGEM_NAME = '" + str.strip(self.Gemeinde) + "') ORDER BY KGEM_GESNR")

##                elif self.rbName.isChecked():

##                    self.abfrage2.exec_("SELECT DISTINCT kgnrkgname FROM kat_gem_vlbg WHERE (PGEM_NAME = '" + str.strip(self.Gemeinde) + "') ORDER BY KGEM_NAME")
...
##            self.modell_kg.setQuery(self.abfrage2)
    ...
    def gstsuche(self):
       ...
       #Am Filesystem gibts keine Sonderzeichen!
        ...
        self.dbaseTabelle = QgsVectorLayer(Pfad + "GST.shp", "Kasperle","ogr")
        ...
        progress.setLabelText("Suche " + ("Läuft"))
        ...
        iter = self.dbaseTabelle.getFeatures()

        progress.setRange(0,self.dbaseTabelle.featureCount()*len(gstliste))
        ...
        for gst in gstliste:
            ...
            for attr in iter:
                ...
                if attr.attribute("GNR") == str.strip(gst) and attr.attribute("KG") == self.kgnummer:
        ...
        if not fid is None:
            ...
            self.dbaseTabelle.selectByIds(fid)
...
##        fid = self.dbaseTabelle.selectedFeaturesIds()
        ...
        if self.dbaseTabelle.selectedFeatureCount() >= 1: #Eins gefunden, Txtfeld und Zoompunkt festlegen
            ...
            self.zoompunkt = self.dbaseTabelle.boundingBoxOfSelected()
